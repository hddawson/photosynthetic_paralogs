...
# v3: stricter argument parsing, refreshes empty raw caches, grep match diagnostics
...
parse_args <- function() {
  ...
  takes_value <- c(
    "--species", "--species-file", "--gbif-file", "--bio8-tif",
    "--cache-dir", "--out", "--scale", "--min-occurrences",
    "--exclude-publisher-key", "--grep-engine", "--threads", "--block", "--sep"
  )
  flags <- c("--force", "--no-refresh-empty-raw")
  i <- 1L
  while (i <= length(x)) {
    key <- x[[i]]
    if (key %in% flags) {
      if (key == "--force") args$force <- TRUE
      if (key == "--no-refresh-empty-raw") args$refresh_empty_raw <- FALSE
      i <- i + 1L
    } else if (key %in% takes_value) {
      ...
      stop("Unexpected positional argument: ", key,
           "\nDid a line-continuation backslash have trailing spaces?")
    }
  }
...
read_species <- function(args) {
  spp <- character()
  if (!is.null(args$species) && nzchar(args$species)) {
    spp <- c(spp, trimws(unlist(strsplit(args$species, ","))))
  }
  if (!is.null(args$species_file) && nzchar(args$species_file)) {
    if (!file.exists(args$species_file)) stop("Species file not found: ", args$species_file)
    spp <- c(spp, trimws(readLines(args$species_file, warn = FALSE)))
  }
  spp <- spp[nzchar(spp) & !startsWith(spp, "#")]
  # Guard against a malformed shell invocation or stale script that treated the file path as a species.
  if (!is.null(args$species_file)) spp <- spp[spp != args$species_file]
  spp <- unique(spp)
  if (!length(spp)) stop("Provide --species or --species-file")
  message("Species requested: ", paste(spp, collapse = "; "))
  spp
}
...
raw_cache_needs_refresh <- function(sp, cache_dir, force = FALSE, refresh_empty_raw = TRUE) {
  f <- raw_cache_file(sp, cache_dir)
  if (force || !file.exists(f)) return(TRUE)
  if (!refresh_empty_raw) return(FALSE)
...
grep_candidates_to_file <- function(gbif_file, species_needed, cache_dir, args) {
  ...
  if (args$grep_engine == "rg") {
    ...
    # --will-cite suppresses GNU Parallel's interactive citation notice.
    # `grep` returns 1 for blocks with no matches; that is normal, so each job must `|| true`.
    # Quote the whole per-block command to avoid nested shell-quote breakage from tempfile paths.
    per_block_cmd <- sprintf("grep -F -f %s || true", q(pat_file))
    body_cmd <- sprintf(
      "LC_ALL=C parallel --will-cite --pipepart --block %s -j %d -a %s %s",
      q(args$block), args$threads, q(gbif_file), q(per_block_cmd)
    )
  ...
  n_matches <- as.integer(system(sprintf("wc -l < %s", q(tmp_body)), intern = TRUE))
  message("Candidate body lines matched: ", n_matches)
...
message("GBIF file: ", args$gbif_file)
message("Cache dir: ", args$cache_dir)

need_raw <- spp[vapply(spp, raw_cache_needs_refresh, logical(1),
                         cache_dir = args$cache_dir,
                         force = args$force,
                         refresh_empty_raw = args$refresh_empty_raw)]
if (length(need_raw)) {
